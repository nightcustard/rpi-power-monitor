...
class Point():
    def __init__(self, p_type, *args, **kwargs):
        if p_type == 'residual_load':
            ...
            self.p_type  : the type of point [residual_load, solar, net, ct, voltage]
...
def write_to_influx(solar_power_values, residual_load_values, net_power_values, ct0_dict, ct1_dict, ct2_dict, ct3_dict, ct4_dict, ct5_dict, poll_time, length, voltages, current_tariff):
    ...
    avg_home_current = sum(residual_load_values['current']) / length
    ...
    # Create Points
    residual_load = Point('residual_load', power=avg_home_power, current=avg_home_current, tariff=current_tariff, time=poll_time)
